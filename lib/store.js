...
function mergeCollection(a = [], b = []) {
  ...
  for (const r of b) {
    ...
    const tA = new Date(cur.updatedAt || 0).getTime();
    const tB = new Date(r.updatedAt || 0).getTime();
    map.set(r.id, tB > tA ? r : cur);
  }
  const cutoff = Date.now() - TOMBSTONE_DAYS * 86_400_000;
  return [...map.values()].filter((r) => !(r.deletedAt && new Date(r.deletedAt).getTime() < cutoff));
}

export function mergeStates(local, remote) {
  if (!remote) return local;
  const activity = mergeCollection(local.activity, remote.activity)
    .sort((x, y) => new Date(y.ts) - new Date(x.ts))
    .slice(0, ACTIVITY_CAP);
  const checklist = { ...(remote.checklist || {}) };
  for (const [day, items] of Object.entries(local.checklist || {})) checklist[day] = { ...(checklist[day] || {}), ...items };
  return {
    ...local,
    tickets: mergeCollection(local.tickets, remote.tickets),
    assignments: mergeCollection(local.assignments, remote.assignments),
    inventory: mergeCollection(local.inventory, remote.inventory),
    activity,
    checklist,
  };
}
