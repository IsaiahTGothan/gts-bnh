...
// ─── Shared gate for other API routes ──────────────────────────────────────
/**
 * Other routes (Salesforce connector) reuse the team passcode: with a team
 * database attached the `x-gts-key` header must match; in local mode (no
 * database) the deployment is single-station and the route stays open.
 * @returns {Promise<{ok:boolean, status:number, body?:object}>}
 */
export async function authorizeRequest(req) {
  const c = config();
  if (!c) return { ok: true, status: 200 };
  try {
    const row = await withTable(readRow);
    const auth = checkPasscode(row, keyFromRequest(req));
    if (auth.needsSetup) return { ok: false, status: 409, body: { needsSetup: true, error: 'Set the team passcode first (Settings → Team sync)' } };
    if (!auth.ok) { noteFailure(); return { ok: false, status: throttled() ? 429 : 401, body: { locked: true, error: 'unauthorized' } }; }
